...
# Autor: LSX
# Propósito: Gestionar el servicio de sincronización de Traefik
# =====================================================
...
SERVICE_FILE="$SCRIPT_DIR/traefik-sync.service"
DAEMON_SCRIPT="$SCRIPT_DIR/traefik-sync-daemon.sh"
SYSTEM_SERVICE_FILE="/etc/systemd/system/traefik-sync.service"
...
install_service() {
    echo -e "${BLUE}🔧 Instalando servicio traefik-sync...${NC}"
    ...
    systemctl enable traefik-sync.service
...
uninstall_service() {
    echo -e "${YELLOW}🗑️  Desinstalando servicio traefik-sync...${NC}"

    if systemctl is-active --quiet traefik-sync.service; then
        systemctl stop traefik-sync.service
        echo -e "${GREEN}✔ Servicio detenido${NC}"
    fi

    systemctl disable traefik-sync.service 2>/dev/null || true
...
# Control de servicio
start_service() {
    echo -e "${BLUE}▶️  Iniciando servicio traefik-sync...${NC}"
    systemctl start traefik-sync.service || {
        echo -e "${RED}❌ No se pudo iniciar. Verifica con:${NC} systemctl status traefik-sync.service"
        exit 1
    }
    sleep 2
    systemctl is-active --quiet traefik-sync.service &&
        echo -e "${GREEN}✅ Servicio iniciado correctamente${NC}" ||
        echo -e "${RED}❌ Error al iniciar el servicio${NC}"
}

stop_service() { echo -e "${YELLOW}⏹️  Deteniendo...${NC}"; systemctl stop traefik-sync.service; }
restart_service() { echo -e "${BLUE}🔄 Reiniciando...${NC}"; systemctl restart traefik-sync.service; }
status_service() { systemctl status traefik-sync.service; }
...
show_help() {
    cat << EOF
${BLUE}=== GESTIÓN DE TRAEFIK SYNC DAEMON ===${NC}
Uso: sudo $0 [comando]
Comandos:
